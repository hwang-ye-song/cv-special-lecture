...
<img src="./images/metr-time-horizon.png" alt="METR Time Horizon 그래프" width="640">
...
<img src="./images/agent-components.png" alt="에이전트 구성 요소" width="560">
...
<img src="./images/function-calling-flow.png" alt="Function Calling 동작 원리" width="420">
...
<img src="./images/claude-code-harness-flow.png" alt="Claude Code 동작 흐름" width="640">
...
<img src="./images/ide-extension.png" alt="IDE에서 Claude Code 익스텐션 설치" width="640">
...
<img src="./images/login-options.png" alt="Claude Code 로그인 화면" width="480">
...
<img src="./images/mcp-architecture.png" alt="MCP 아키텍처" width="640">
...
- **Agent Skills**: AI 에이전트의 기능을 확장하기 위한 **가볍고 개방적인 포맷**(lightweight, open format)
- 에이전트가 특정 작업을 정확하게 수행하도록 쓸 수 있는 **지침·스크립트·에셋으로 구성된 파일 폴더**
- Skill 하나에 많은 정보를 담을 수 있고, 에이전트는 **수백 개의 skill**을 가질 수 있음

#### 점진적 공개 (Progressive Disclosure)

수백 개의 skill을 매번 전부 읽으면 토큰 낭비이므로, **필요한 단계만 순서대로 로드**함

| 단계 | 내용 | 언제 로드되나 |
|---|---|---|
| **메타데이터** | YAML frontmatter (`name`, `description`) | **항상** |
| **지침** | `SKILL.md` 본문 | skill이 **트리거될 때** |
| **리소스** | `references/` 등 추가 파일 | **필요할 때** |

<img src="./images/progressive-disclosure.png" alt="Progressive Disclosure" width="600">

#### SKILL.md 구조

`SKILL.md` = **YAML Frontmatter** + **Markdown 본문**

- `name`: 영문 소문자·숫자·하이픈만, **상위 폴더 이름과 같아야 함**
- `description`: **무엇을 하는지 + 언제 쓰는지**를 함께 적어야 에이전트가 알아서 찾아 씀
- 본문은 **500줄 이하**로 두고, 자세한 자료는 `references/`, 실행 파일은 `scripts/`, 템플릿·이미지는 `assets/`로 분리

#### 왜 쓰는가

- Skills가 없으면 매번 지침을 설명하고, 참조 파일을 묶어 주고, 결과가 일관적인지 확인해야 함
- Skills는 **절차적 지식과 회사·팀·개인의 문맥**을 필요할 때 불러오게 해 줌
- 여러 skill을 **결합**해 복잡한 워크플로우를 만들 수도 있고, Claude Code·Claude.ai·API 등 **여러 제품에서 같은 skill을 그대로** 사용 가능

<img src="./images/skills-composable.png" alt="Skills 조합 예시" width="600">

#### 세팅 방법

프로젝트 안에 skills 폴더를 만들고 skill 폴더를 넣음 (교안은 Codex 기준 `.codex/skills/`, Claude Code는 `.claude/skills/`)
...
**다른 사람의 GitHub Skills 가져오기** (IDE 터미널에서 실행)

```bash
git clone https://github.com/jh941213/my-claude-code-asset.git
mkdir -p .codex/skills
cp -R my-claude-code-asset/skills/* .codex/skills/
rm -rf my-claude-code-asset
```

> 참고: [Anthropic Skills 가이드](https://resources.anthropic.com/hubfs/The-Complete-Guide-to-Building-Skill-for-Claude.pdf) · [Vercel React skills](https://github.com/vercel-labs/agent-skills/tree/main/skills/react-best-practices) · [Skills·플러그인 가이드](https://skills-plugins-guide.vercel.app/)
